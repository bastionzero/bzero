...
func (d *DefaultSsh) Receive(action string, actionPayload []byte) ([]byte, error) {
	...
	case ssh.SshOpen:
		...
		go func() {
			...
			if err := d.clearAuthorizedKeyEntry(); err != nil {
...
func (d *DefaultSsh) handleOpenShellDataAction(openRequest ssh.SshOpenMessage) error {
	...
	d.authorizedKeyEntry = fmt.Sprintf("%s %s %s", keyType, keyContents, comment)
	...
	*/
	...
	d.logger.Infof("Adding authorized key entry %s for user: %s", d.authorizedKeyEntry, d.targetUser)
	var keyAdded, err = addToAuthorizedKeyFile(d.targetUser, d.authorizedKeyEntry)
	if !keyAdded {
...
func (d *DefaultSsh) clearAuthorizedKeyEntry() error {
	...
	for scanner.Scan() {
		if scanner.Text() != d.authorizedKeyEntry {
	...
	if !foundKey {
		d.logger.Debugf("tried to remove this session's entry from /home/%s/.ssh/authorized_keys, but it was already deleted", d.targetUser)
	} else {
		d.logger.Debugf("removed this session's entry from /home/%s/.ssh/authorized_keys", d.targetUser)
	}
